        ...
        <div className="flex-1 p-2 border rounded-lg border-font-color-light border-opacity-20">
          ...
          {comment.isLiked ? (
            heartIconHover === comment?.id ? (
              <>
                <p className="inline p-1 text-sm font-semibold text-font-color-light">
                  {comment?.numLikes}
                </p>
                <BsHeart
          ...
          {session?.user ? (
            <button
              onClick={() => {
                showReply === comment?.id ? setShowReply(false) : setReply("");
                setShowReply(comment?.id);
                replyRef.current?.focus();
              }}
              className="text-sm text-primary ml-1.5 font-semibold hover:underline"
            >
              Reply
            </button>
          ) : null}
            ...
            <div className="flex w-1/3 space-x-2">
              <div className="flex flex-col items-center space-y-2">
                <img
                  ...
                  alt=""
                />
                <p className="text-xs font-semibold text-font-color-light">
                  {session?.user?.name}
                </p>
              </div>
              <form
